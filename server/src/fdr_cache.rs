...
pub struct FdrCache {
    ...
    podcasts_by_tag: HashMap<PodcastTag, HashSet<Podcast>>,
}
...
impl FdrCache {
    ...
    fn new(podcasts: Vec<Podcast>) -> Self {
        let mut podcasts_by_tag: HashMap<PodcastTag, HashSet<Podcast>> = HashMap::new();
        podcasts.iter().for_each(|podcast| {
            podcast.get_tags().iter().for_each(|tag| {
    ...
    pub fn get_filtered_tags_with_podcast_counts(
        &self,
        exclusive_podcasts_or: Option<HashSet<Podcast>>,
        tags: Vec<PodcastTag>,
    ...
    fn get_podcasts_by_tags(&self, mut tags: Vec<PodcastTag>) -> Vec<Podcast> {
        if tags.is_empty() {
            ...
            for podcast in self.num_sorted_podcast_list.iter() {
                all_podcasts.push(podcast.clone());
            }
            return all_podcasts;
        }
        ...
        let mut podcasts: HashSet<Podcast> = match first_tag_podcasts_or {
            Some(first_tag_podcasts) => {
                let mut first_tag_podcasts_copy = HashSet::new();
                for podcast in first_tag_podcasts.iter() {
                    first_tag_podcasts_copy.insert(podcast.clone());
                }
