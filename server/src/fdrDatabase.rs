...
impl FdrDatabase {
    ...
    async fn get_podcast(&self, find_doc: Document) -> mongodb::error::Result<Option<Podcast>> {
        ...
        let doc_or = self.database.collection("podcasts").find_one(find_doc, None).await?;
        Ok(match doc_or {
    ...
    pub async fn get_podcast_by_num_i32(&self, num: i32) -> mongodb::error::Result<Option<Podcast>> {
        self.get_podcast(doc!{"num": num}).await
    }

    // TODO - Make sure that this works correctly with MongoDB `number` *and* `double`.
    pub async fn get_podcast_by_num_f64(&self, num: f64) -> mongodb::error::Result<Option<Podcast>> {
        self.get_podcast(doc!{"num": num}).await
    }

    pub async fn get_all_podcasts(&self) -> mongodb::error::Result<Vec<Podcast>> {
        // TODO - Add projection doc.
        let cursor = self.database.collection("podcasts").find(None, None).await?;
        let docs_or = cursor
            .collect::<Vec<Result<Document, mongodb::error::Error>>>()
            .await;
        let mut podcasts: Vec<Podcast> = docs_or.iter().filter_map(|doc_or| {
            match doc_or {
        ...
        podcasts.sort_by(|a, b| {
            if a.get_num().as_f64() > b.get_num().as_f64() {
